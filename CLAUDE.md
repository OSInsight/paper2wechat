...
如果你是 **CodeX** 或其他不会自动扫描 `.claude/` 目录的通用编程助手：

1. **当用户请求“把 arXiv 论文转公众号文章 / 转 WeChat”时，必须先读取并遵循：**
   - `.claude/skills/paper2wechat/SKILL.md`
2. 严格按该 Skill 的流程执行：
   - 先运行 `python -m paper2wechat.core.fetch_cli <url_or_id_or_pdf> --verbose` 生成解析缓存与图片
   - 再根据用户意图（style/长度/重点）生成公众号文章并保存到 `outputs/<paper_id>.md`
3. 如果你无法读取 `.claude/skills/...`（工具限制），请在回复里明确说明限制，并改为：
   - 参考本仓库 `README.md` / `docs/INTEGRATION.md` 的使用方式完成最小可用流程
